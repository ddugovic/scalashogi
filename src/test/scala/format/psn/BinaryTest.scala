...
class BinaryTest extends ShogiTest {
  ...
  def compareStrAndBin(psn: String) = {
    val bin = (Binary writeMoves psn.split(' ').toList).get.toList
    ((Binary readMoves bin).get mkString " ") must_== psn
    bin.size must be_<=(psn.length)
  }
  ...
  "binary encoding" should {
    ...
    "write many moves" in {
      "all games" in {
        forall(psn200) { psn =>
          val bin = (Binary writeMoves psn.split(' ').toList).get
          bin.length must be_<=(psn.length)
        }
      }
    }
    "read single move" in {
    ...
    "be isomorphic" in {
      "for one" in {
        compareStrAndBin(psn200.head)
      }
      "for all" in {
        forall(psn200)(compareStrAndBin)
      }
    }
  }
...
object BinaryTestData {
  ...
  val psn200: List[String] = augmentString(
    """
